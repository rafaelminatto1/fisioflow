...
import { Exercise, ExerciseImage, ImageCategory, ImageAnnotation } from '../../types';
...
const ImageGalleryModal: React.FC<ImageGalleryModalProps> = ({
  ...
  const { user } = useAuth();
  const { getExerciseImages, getExerciseImagesByCategory, updateExerciseImage } = useData();
  ...
  const selectedImage = allImages.find(img => img.id === selectedImageId) || filteredImages[0];

  const handleUpdateAnnotations = (annotations: ImageAnnotation[]) => {
    if (!user || !selectedImage) return;
    
    updateExerciseImage(selectedImage.id, { annotationPoints: annotations }, user);
  };
  ...
  const currentImageIndex = selectedImage ? filteredImages.findIndex(img => img.id === selectedImage.id) + 1 : 0;
    ...
    return (
      <BaseModal isOpen={isOpen} onClose={onClose} title="Galeria de Imagens">
        <div className="py-8 text-center">
          ...
          <h3 className="mb-2 font-semibold text-slate-900">Nenhuma imagem disponível</h3>
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          {categories.map((category) => {
            const count = category.value === 'all' 
              ? allImages.length 
              : getExerciseImagesByCategory(exercise.id, category.value as ImageCategory).length;
        ...
        {filteredImages.length === 0 ? (
          <div className="py-8 text-center">
            <div className="text-4xl mb-4">🔍</div>
            <h3 className="mb-2 font-semibold text-slate-900">Nenhuma imagem nesta categoria</h3>
            <p className="text-slate-600">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2">
              {selectedImage && (
                <div className="space-y-4">
                  {/* Image Container */}
                  <div 
                    className={`relative rounded-lg bg-slate-100 overflow-hidden transition-all duration-200 ${
                      isZoomed ? 'fixed inset-4 z-50 bg-black bg-opacity-90' : 'aspect-video'
                    }`}
                  >
                    <div 
                      className={`w-full h-full ${!isZoomed ? 'cursor-pointer' : ''}`}
                    ...
                    {filteredImages.length > 1 && (
                      <>
                        <button
                          ...
                          className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-black bg-opacity-50 p-2 text-white hover:bg-opacity-75 transition-all"
                        >
                          ←
                        </button>
                        <button
                          ...
                          className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-black bg-opacity-50 p-2 text-white hover:bg-opacity-75 transition-all"
                        >
                          →
                        </button>
                      </>
                    ...
                    <div className="absolute bottom-2 right-2 rounded bg-black bg-opacity-50 px-2 py-1 text-xs text-white">
                      {isZoomed ? '🔍 Clique para diminuir' : '🔍 Clique para ampliar'}
                    </div>
                      ...
                      <div>
                        ...
                        <div className="mt-1 flex items-center space-x-3 text-sm text-slate-500">
                          <span className="flex items-center">
                            {getCategoryIcon(selectedImage.category)} {getCategoryLabel(selectedImage.category)}
                          </span>
                          <span>📅 {new Date(selectedImage.uploadedAt).toLocaleDateString('pt-BR')}</span>
                          {selectedImage.annotationPoints && selectedImage.annotationPoints.length > 0 && (
                            <span className="flex items-center">
                              📍 {selectedImage.annotationPoints.length} marcaç{selectedImage.annotationPoints.length === 1 ? 'ão' : 'ões'}
                            </span>
                          )}
                        </div>
                      </div>
            ...
            <div className="lg:col-span-1">
              ...
              <div className="grid grid-cols-2 gap-2 lg:grid-cols-1 max-h-96 overflow-y-auto">
                {filteredImages.map((image) => (
                  <button
                    key={image.id}
                    onClick={() => handleImageSelect(image)}
                    className={`group relative rounded-lg border-2 overflow-hidden transition-all duration-200 ${
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-200 flex items-end">
                      <div className="w-full bg-black bg-opacity-75 text-white p-2 transform translate-y-full group-hover:translate-y-0 transition-transform duration-200">
