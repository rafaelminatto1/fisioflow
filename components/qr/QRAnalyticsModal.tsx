...
const QRAnalyticsModal: React.FC<QRAnalyticsModalProps> = ({
  ...
  const analytics = useMemo(() => {
    ...
    return qrCodeService.getAnalytics(user.tenantId)
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} title="Analytics de QR Code" size="large">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          <div className="rounded-lg bg-blue-50 p-4 text-center">
            <div className="text-2xl font-bold text-blue-700">{stats.totalAccesses}</div>
            <div className="text-sm text-blue-600">Total de Acessos</div>
          </div>
          
          <div className="rounded-lg bg-green-50 p-4 text-center">
            <div className="text-2xl font-bold text-green-700">{stats.uniqueQRCodes}</div>
        ...
        {/* Device Breakdown */}
        <div>
          <h4 className="mb-3 font-semibold text-slate-900">Dispositivos Utilizados</h4>
          <div className="space-y-2">
            {Object.entries(stats.deviceBreakdown).map(([device, count]) => {
              const percentage = stats.totalAccesses > 0 
        ...
        {/* Recent Access Log */}
        <div>
          <h4 className="mb-3 font-semibold text-slate-900">Acessos Recentes</h4>
                ...
                {analytics.map((access) => (
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <span className="text-lg">{getDeviceIcon(access.deviceType)}</span>
                      <div>
                        <div className="text-sm font-medium text-slate-900">
                          QR ID: {access.qrCodeId.substring(0, 12)}...
                        </div>
                        <div className="text-xs text-slate-500">
                          {getDeviceLabel(access.deviceType)} • {formatDate(access.accessedAt)}
                        </div>
                      </div>
                    </div>
        ...
        <div className="border-t pt-4">
          <div className="flex items-center justify-between">
            <div className="text-sm text-slate-600">
              💡 Os dados são atualizados em tempo real conforme os pacientes acessam os QR Codes
            </div>
            <div className="flex space-x-2">
              <button
                onClick={() => {
