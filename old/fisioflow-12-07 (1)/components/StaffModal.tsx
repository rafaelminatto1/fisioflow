...
const StaffModal: React.FC<StaffModalProps> = ({ isOpen, onClose, onSave, onDelete, user: userToEdit }) => {
  usePreventBodyScroll(isOpen);
  const { user: loggedInUser } = useAuth();
  const [editedUser, setEditedUser] = useState<Partial<User> | null>(userToEdit);
  ...
  const handleDelete = () => {
    if (editedUser && 'id' in editedUser && window.confirm(`Tem certeza que deseja excluir o usuário "${editedUser.name}"?`)) {
  ...
  return (
    ...
    >
        ...
        {isEditingSelf && <p className="text-xs text-slate-400 -mt-2">Você não pode alterar seu próprio cargo.</p>}
    </BaseModal>
  );
};
