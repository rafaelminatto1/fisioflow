...
class FeedbackService {
  ...
  async submitFeedback(
    feedback: Omit<FeedbackEntry, 'id' | 'timestamp' | 'resolved'>
  ): Promise<string> {
    try {
      ...
      aiLogger.error('FEEDBACK_SERVICE', 'Erro ao registrar feedback', {
        error,
        feedback,
      });
      throw error;
    ...
    if (tenantId) {
      ...
      const filteredMetrics = metrics.filter((metric) => {
        const feedbacks = Array.from(this.feedbacks.values()).filter(
          (f) => f.knowledgeEntryId === metric.entryId
        );
  ...
  async createImprovementSuggestion(
    ...
    try {
      ...
      aiLogger.error('FEEDBACK_SERVICE', 'Erro ao criar sugestão de melhoria', {
        error,
        suggestion,
      });
      throw error;
  ...
  async getImprovementSuggestions(filters?: {
    ...
    if (filters) {
      ...
      if (filters.assignedTo) {
        suggestions = suggestions.filter(
          (s) => s.assignedTo === filters.assignedTo
        );
      }
    }
  ...
  async updateSuggestionStatus(
    ...
    try {
      ...
      aiLogger.error(
        'FEEDBACK_SERVICE',
        'Erro ao atualizar status de sugestão',
        {
          error,
          suggestionId,
          status,
  ...
  async resolveFeedback(feedbackId: string, resolvedBy: string): Promise<void> {
    try {
      ...
      aiLogger.error('FEEDBACK_SERVICE', 'Erro ao resolver feedback', {
        error,
        feedbackId,
      });
      throw error;
  ...
  async generateQualityReport(tenantId?: string): Promise<{
    ...
    try {
      ...
      const allFeedbacks = Array.from(this.feedbacks.values()).filter(
        (f) => !tenantId || f.tenantId === tenantId
      );
      ...
      const averageRating =
        allMetrics.length > 0
          ? allMetrics.reduce((sum, m) => sum + m.averageRating, 0) /
            allMetrics.length
          : 0;
      ...
      const lowRatedEntries = sortedByRating
        .slice(-10)
        .reverse()
        .map((m) => ({
          entryId: m.entryId,
      ...
      aiLogger.error(
        'FEEDBACK_SERVICE',
        'Erro ao gerar relatório de qualidade',
        { error, tenantId }
      );
      throw error;
    }
  ...
  private async updateQualityMetrics(entryId: string): Promise<void> {
    try {
      ...
      const averageRating =
        entryFeedbacks.reduce((sum, f) => sum + f.rating, 0) / totalFeedbacks;

      const helpfulCount = entryFeedbacks.filter(
        (f) => f.type === 'helpful'
      ).length;
      const notHelpfulCount = entryFeedbacks.filter(
        (f) => f.type === 'not_helpful'
      ).length;
      const incorrectCount = entryFeedbacks.filter(
        (f) => f.type === 'incorrect'
      ).length;
      const outdatedCount = entryFeedbacks.filter(
        (f) => f.type === 'outdated'
      ).length;
      const suggestionCount = entryFeedbacks.filter(
        (f) => f.type === 'suggestion'
      ).length;

      // Calcular score de confiança baseado em múltiplos fatores
      const helpfulRatio =
        totalFeedbacks > 0 ? helpfulCount / totalFeedbacks : 0.5;
      const ratingRatio = averageRating / 5;
      const problemRatio =
        totalFeedbacks > 0
          ? (incorrectCount + outdatedCount) / totalFeedbacks
          : 0;
      ...
      aiLogger.error(
        'FEEDBACK_SERVICE',
        'Erro ao atualizar métricas de qualidade',
        {
          error,
          entryId,
        }
  ...
  async getEntryTrends(
    ...
    const entryFeedbacks = (await this.getFeedbacksForEntry(entryId)).filter(
      (f) => new Date(f.timestamp) >= cutoffDate
    );
  ...
  async generateAutomaticSuggestions(
    entryId: string
  ): Promise<ImprovementSuggestion[]> {
    const suggestions: Omit<
      ImprovementSuggestion,
      'id' | 'suggestedAt' | 'status'
    >[] = [];
