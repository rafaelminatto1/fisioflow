...
import { AIQuery, AIResponse, ResponseSource, PremiumProvider, SavingsReport } from '../../types/ai-economica.types';
...
export class AnalyticsService {
  ...
  recordQuery(query: AIQuery, response: AIResponse, source: ResponseSource): void {
    const analytics: QueryAnalytics = {
      ...
      actualCost: source === ResponseSource.PREMIUM ? this.calculateActualCost(response) : 0,
    ...
    aiEconomicaLogger.logQuery(query.id, query.type, source, query.context.userId, query.context.tenantId);
  }

  getUsageMetrics(period?: { start: Date; end: Date }, tenantId?: string): UsageMetrics {
    ...
    // Calcular métricas
    const queriesBySource = filteredHistory.reduce((acc, q) => {
      acc[q.source] = (acc[q.source] || 0) + 1;
      return acc;
    }, {} as Record<ResponseSource, number>);

    const queriesByType = filteredHistory.reduce((acc, q) => {
      acc[q.queryType] = (acc[q.queryType] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const averageResponseTime = filteredHistory.reduce((sum, q) => sum + q.responseTime, 0) / filteredHistory.length;
    const averageConfidence = filteredHistory.reduce((sum, q) => sum + q.confidence, 0) / filteredHistory.length;
    ...
    const cacheHitRate = filteredHistory.length > 0 ? cacheHits / filteredHistory.length : 0;

    return {
      totalQueries: filteredHistory.length,
      queriesBySource: {
        [ResponseSource.INTERNAL]: queriesBySource[ResponseSource.INTERNAL] || 0,
  ...
  generateSavingsReport(month?: string): SavingsReport {
    ...
    const queriesAnsweredInternally = monthlyQueries.filter(
      q => q.source === ResponseSource.INTERNAL || q.source === ResponseSource.CACHE
    ).length;
    ...
    const cacheHits = monthlyQueries.filter(q => q.source === ResponseSource.CACHE).length;
    const cacheHitRate = monthlyQueries.length > 0 ? cacheHits / monthlyQueries.length : 0;
  ...
  getTopQueries(limit = 10, period?: { start: Date; end: Date }): Array<{
    queryType: string;
    count: number;
    averageResponseTime: number;
    averageConfidence: number;
    ...
    const queryTypeStats = filteredHistory.reduce((acc, q) => {
      if (!acc[q.queryType]) {
        acc[q.queryType] = {
          count: 0,
          totalResponseTime: 0,
    ...
    return Object.entries(queryTypeStats)
      .map(([queryType, stats]) => {
        const preferredSource = Object.entries(stats.sourceCount)
          .reduce((a, b) => stats.sourceCount[a[0]] > stats.sourceCount[b[0]] ? a : b)[0] as ResponseSource;
  ...
  getPerformanceTrends(days = 30): Array<{
    ...
    return Object.values(trends).map((day: any) => ({
      ...
      averageResponseTime: day.totalQueries > 0 ? day.totalResponseTime / day.totalQueries : 0,
      cacheHitRate: day.totalQueries > 0 ? day.cacheHits / day.totalQueries : 0,
      internalSuccessRate: day.totalQueries > 0 ? day.internalSuccesses / day.totalQueries : 0
    }));
  }

  getUserAnalytics(userId: string, period?: { start: Date; end: Date }): {
    totalQueries: number;
    ...
    const queryTypeCounts = userQueries.reduce((acc, q) => {
      acc[q.queryType] = (acc[q.queryType] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    ...
    // Fontes preferidas
    const sourceCounts = userQueries.reduce((acc, q) => {
      acc[q.source] = (acc[q.source] || 0) + 1;
      return acc;
    }, {} as Record<ResponseSource, number>);
    ...
    const averageConfidence = userQueries.reduce((sum, q) => sum + q.confidence, 0) / userQueries.length;
  ...
  private estimateQueryCost(query: AIQuery): number {
    ...
    const multiplier = typeMultiplier[query.type as keyof typeof typeMultiplier] || 1.0;
  ...
  clearOldData(daysToKeep = 90): number {
    ...
    this.queryHistory = this.queryHistory.filter(q => q.timestamp >= cutoffDate);
    const removedCount = initialCount - this.queryHistory.length;

    if (removedCount > 0) {
      this.saveToStorage();
      aiEconomicaLogger.log('INFO', 'ANALYTICS_CLEANUP', `Removed ${removedCount} old analytics records`);
    }
  ...
  exportData(format: 'json' | 'csv' = 'json'): string {
    ...
    // CSV format
    const headers = [
      'queryId', 'timestamp', 'queryType', 'source', 'provider',
      'responseTime', 'confidence', 'tokensUsed', 'estimatedCost',
      'actualCost', 'userId', 'tenantId'
    ].join(',');
