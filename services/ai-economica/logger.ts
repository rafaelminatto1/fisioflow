...
export class AIEconomicaLogger {
  ...
  private log(
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(
        `[AI-ECONOMICA] ${level.toUpperCase()}: ${category} - ${message}`,
        data
      );
    }
  }
  ...
  logCacheCleanup(removedEntries: number): void {
    this.log(
      LogLevel.INFO,
      'CACHE_CLEANUP',
      `Removed ${removedEntries} expired cache entries`
    );
  }
  ...
  logPremiumLimitWarning(
    ...
    this.log(
      LogLevel.WARN,
      'PREMIUM_LIMIT',
      `${provider} usage at ${usagePercentage}%`,
      {
        provider,
        usagePercentage,
      }
  ...
  logPremiumLimitReached(provider: PremiumProvider): void {
    this.log(LogLevel.ERROR, 'PREMIUM_LIMIT', `${provider} limit reached`, {
      provider,
    });
  }
  ...
  logCostSaving(
    ...
    this.log(
      LogLevel.INFO,
      'COST_SAVING',
      `Estimated saving: R$ ${estimatedSaving.toFixed(2)}`,
      {
        estimatedSaving,
        source,
      },
  ...
  logSlowQuery(queryId: string, responseTime: number, threshold: number): void {
    this.log(
      LogLevel.WARN,
      'SLOW_QUERY',
      `Slow query detected: ${responseTime}ms (threshold: ${threshold}ms)`,
      {
        queryId,
        responseTime,
        threshold,
  ...
  cleanupOldLogs(): void {
    ...
    retentionDate.setDate(
      retentionDate.getDate() - AI_ECONOMICA_CONFIG.LOGGING.RETENTION_DAYS
    );
    ...
    if (removedCount > 0) {
      this.log(
        LogLevel.INFO,
        'LOG_CLEANUP',
        `Removed ${removedCount} old log entries`
      );
    }
  }
}
