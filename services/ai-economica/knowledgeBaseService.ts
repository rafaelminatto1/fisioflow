...
export class KnowledgeBaseService {
  ...
  async addKnowledge(
    ...
    // Log da contribuição
    aiEconomicaLogger.logKnowledgeContribution(
      fullEntry.id,
      fullEntry.author.id,
      fullEntry.tenantId
    );
  ...
  async search(params: SearchParams): Promise<KnowledgeResult[]> {
    try {
      // Busca por texto
      const textResults = await this.searchEngine.searchByText(
        params.text,
      ...
      const symptomResults = params.symptoms
        ? await this.searchEngine.searchBySymptoms(
            params.symptoms,
            params.context?.tenantId
          )
        : [];
      ...
      const diagnosisResults = params.diagnosis
        ? await this.searchEngine.searchByDiagnosis(
            params.diagnosis,
            params.context?.tenantId
          )
        : [];
  ...
  async updateConfidence(
    ...
    const newConfidence = Math.max(
      0.1,
      Math.min(1.0, entry.confidence + adjustment)
    );
  ...
  private validateEntry(entry: any): void {
    ...
    if (
      !entry.type ||
      !['protocol', 'exercise', 'case', 'technique', 'experience'].includes(
        entry.type
      )
    ) {
  ...
  private calculateInitialConfidence(entry: any): number {
    ...
    if (entry.contraindications && entry.contraindications.length > 0)
      confidence += 0.05;
  ...
  private rankResults(
    ...
    return uniqueResults
      .sort((a, b) => {
        ...
        const daysSinceUsedA =
          (Date.now() - new Date(a.entry.lastUsed).getTime()) /
          (1000 * 60 * 60 * 24);
        const daysSinceUsedB =
          (Date.now() - new Date(b.entry.lastUsed).getTime()) /
          (1000 * 60 * 60 * 24);
        ...
        if (
          params.context?.specialty &&
          a.entry.metadata.specialty.includes(params.context.specialty)
        ) {
          scoreA += 0.15;
        }
        if (
          params.context?.specialty &&
          b.entry.metadata.specialty.includes(params.context.specialty)
        ) {
          scoreB += 0.15;
        }
  ...
  async rebuildIndex(tenantId?: string): Promise<void> {
    ...
    aiEconomicaLogger.log(
      'INFO',
      'KNOWLEDGE_BASE',
      `Index rebuilt for ${entries.length} entries`,
      { tenantId }
    );
  }
  ...
  async cleanupOldEntries(daysOld = 365, tenantId?: string): Promise<number> {
    ...
    const oldEntries = await this.contentManager.findOlderThan(
      cutoffDate,
      tenantId
    );
    let removedCount = 0;
    ...
    aiEconomicaLogger.log(
      'INFO',
      'KNOWLEDGE_BASE',
      `Cleaned up ${removedCount} old entries`,
      { tenantId }
    );
    return removedCount;
  }
